...
class flix:
    ...
    def get_sequence_revisions(self,
        ...
        """
        url = '/show/{0}/sequence/{1}/revisions'.format(show_id, seq_id)
        if episode_id is not None:
            url = '/show/{0}/episode/{1}/sequence/{2}/revisions'.format(
                show_id, episode_id, seq_id)
        headers = self.__get_headers(None, url, 'GET')
        response = None
        try:
            r = requests.get(self.hostname + url, headers=headers,
                             verify=False)
            response = json.loads(r.content)
            response = response.get('sequence_revisions')
        except requests.exceptions.RequestException as err:
            if r is not None and r.status_code == 401:
                print('Your token has been revoked')
            else:
